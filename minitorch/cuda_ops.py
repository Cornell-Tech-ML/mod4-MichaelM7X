...
# This code will CUDA compile fast versions your tensor data functions.
...
def tensor_map(fn: Callable[[float], float]) -> Any:
    ...
    Args:
        ...
        out (Stoage): storage for out tensor.
        out_shape (Shape): shape for out tensor. 
        out_strides (Strides): strides for out tensor. 
        out_size (in): size for out tensor.
...
def _tensor_matrix_multiply( 
    ...
    * Only write to global memory once per kernel.
    ...
    for k_start in range(0, a_shape[2], BLOCK_DIM):
        k = k_start + pj
        if i < a_shape[i] and k < a_shape[2]:
    ...
    if i < out_shape[1] and j < out_shape[2]:
        out[out_strides[0] * batch + out_strides[1] * i + out_strides[2] *      = accum
    # END ASSIGN3.4
