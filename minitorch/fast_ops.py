...
class FastOps(TensorOps):
    ...
    def matrix_multiply(a: Tensor, b: Tensor) -> Tensor:
        ...
        is = list(shape_broadcast(a.shape[:-2], b.shape[:-2]))
        ls.append(a.shape[-2]) 
        ls.append(b.shape[-1]) 
        assert a.shape[-1] b.shape[-2]
...
def tensor map(fn: Callable[[float], float]) -> Any:
    ...
    * When out and in are stride-aligned, avoid indexing 
    ...
    return njit(paralle1=True)(_map)
...
def tensor_zip(fn: Callable[[float, float], float]) -> Any:
    ...
    * When 'out', 'a', 'b' are stride-aligned, avoid indexing
    ...
    return njit(paralle1=True)(_zip)
...
def _tensor_matrix_multiply( 
    ...
    * Inner loop should have no global writes, 1 multiply. 
    ...
    b_batch_stride = bstrides[0] if b_shape[0] > 1 else 0

    # ASSIGN3.2
    for it in prange(out_shape[0]):
        for i2 in prange(out_shape[1]):
            for i3 in prange(out_shape[2]):
                a_inner = it * a_batch_stride + i2 * a_strides[1] 
                b_inner = it * b_batch_stride + i3 * b_strides[2]
                acc = 0.0
                for _ in range(a_shape[2]):
                    ...
                    b_inner += b_strides[1] 
                out_position =
                    it * out            + i2 * out           + i3 * out strides[2]

                out[out_position] = acc 
    # END ASSIGN3.2


tensor_matrix_multiply = njit(paralle1=True, fastmath=True)(_tensor_matrix_multiply)

